...
class ComplementArb(Strategy):
    ...
    def generate(self, snap: MarketSnapshot) -> list[Signal]:
        ...
        for m in snap.markets:
            ...
            # Taker-Gebühr (2026): rate * p * (1-p) pro Share, auf beiden Beinen
            fee_rate = self.cfg.risk.taker_fee_rate
            fees = fee_rate * (ya.price * (1 - ya.price) + na.price * (1 - na.price))
            edge = 1.0 - cost - fees
            if edge < min_edge:
                continue

            # Größe: begrenzt durch beide Ask-Level und das Order-Limit
            size = min(ya.size, na.size, max_order / max(cost, 1e-9))
